...
@tool(
    ...
    description=(
        ...
        "The result also includes 'viewport' (viewport CSS-pixel size, or "
        "null if unknown), 'image' (pixel dimensions of the returned "
        "image — may differ from the viewport on a high-DPI/mobile "
        "fingerprint or when scale<1.0), and 'click_scale' (the factor to "
        "multiply an image pixel by to get a browser_click_xy coordinate). "
        "To click something you SEE in the image at pixel (px, py), call "
        "browser_click_xy(px * click_scale.x, py * click_scale.y). On the "
        "default desktop profile these are 1:1, so you can click the "
        "pixel directly. 'click_scale' is provided ONLY for viewport "
        "screenshots (full_page=false); for full-page captures it is "
        "omitted — scroll the target into view and take a viewport "
        "screenshot before using click_xy."
    ),
    parameters={
        "full_page": {
            "type": "boolean",
...
@tool(
    ...
    description=(
        ...
        "Coordinates are VIEWPORT CSS pixels — top-left origin, "
        "devicePixelRatio (DPR) ignored — NOT device pixels and NOT raw "
        "screenshot-image pixels. Viewport-relative, and bounded by the "
        "current viewport (a browser_screenshot result's 'viewport' field "
        "reports its exact width/height — fleet resolutions vary, e.g. "
        "1920x1080, 1366x768, 1536x864). "
        "If you're clicking something you SEE in a browser_screenshot "
        "image, its 'click_scale' field maps an image pixel to this "
        "coordinate space: click_xy(px * click_scale.x, py * "
        "click_scale.y) — needed whenever the fingerprint's device scale "
        "factor isn't 1 or the screenshot was taken with scale<1.0. "
