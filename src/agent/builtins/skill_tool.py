...
@skill(
    ...
    description=(
        ...
        "To send notifications or access mesh/workspace, declare them as keyword "
        "parameters — the framework injects them automatically:\n"
        "  mesh_client   → call await mesh_client.notify_user(msg) to notify the user\n"
        "  workspace_manager → access agent workspace\n"
        "  memory_store  → access agent memory\n"
        "IMPORTANT: Do NOT import other tool modules (e.g. notify_user, mesh_tool). "
        "Use injected parameters instead. Functions using await must be 'async def'.\n"
        "Example (simple):\n"
        "  from src.agent.skills import skill\n"
        ...
        "  from src.agent.skills import skill\n"
        "  @skill(name='ping_user', description='Pings the user', parameters={})\n"
        "  async def ping_user(*, mesh_client=None) -> dict:\n"
        "      if mesh_client:\n"
        "          await mesh_client.notify_user('Ping!')\n"
        "      return {'sent': True}"
    ),
    parameters={
        "name": {
