...
- **Browser container network egress filter.** The shared browser service container runs with `CAP_NET_ADMIN` solely so its root init phase can install an iptables egress filter (`docker/browser-entrypoint.sh`) that REJECTs outbound traffic to RFC1918 / loopback / link-local / CGNAT / IANA-reserved ranges and IPv6 equivalents. The long-running browser runs as UID 1000 via `gosu` and cannot modify the rules. This is the authoritative SSRF control for browser-initiated traffic — the mesh-side `_resolve_and_pin()` check is kept as a friendly early-reject. Filter is skipped (with a loud warning) in host-network mode. Operator allowlist via `BROWSER_EGRESS_ALLOWLIST=cidr,...`; full disable via `BROWSER_EGRESS_DISABLE=1`.
