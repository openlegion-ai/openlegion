...
| `workspace.py` | Persistent markdown workspace. Bootstrap files: PROJECT.md, SYSTEM.md, INSTRUCTIONS.md, SOUL.md, USER.md, MEMORY.md, INTERFACE.md. Also manages HEARTBEAT.md, daily logs, learnings. |
...
| `operator_tools.py` | Operator-only fleet/project orchestration. `read_agent_config` (canonical inverse of `edit_agent` — returns the same field surface so the operator can review current values before mutating; optional `fields=[...]` subset), `list_peer_artifacts` / `read_peer_artifact` (operator can read peer-written artifacts via `GET /mesh/agents/{id}/artifacts[/{name}]`; mirrors the existing dashboard peer-artifact read path, 5 MB cap, path-traversal rejected mesh-side), `edit_agent` (one tool — ALL fields apply immediately and emit an undo receipt: 5-min for soft fields, 30-min for hard fields; no pre-execution confirm step; `heartbeat_schedule` retargets the live cron job), `undo_change`, deprecated stubs `propose_edit` (routes through `edit_agent`) / `confirm_edit` (no-op), `cancel_pending_action`, `archive_audit_before`, `save_observations`, `inspect_agents` (optional `stale_threshold_hours: int`), `inspect_projects`, `create_agent`, `create_project` (provenance gate dropped — operator can spawn autonomously), `add_agents_to_project`, `remove_agents_from_project`, `update_project_context`, `set_project_goal`, `manage_agent`, `manage_project`, `manage_task`. `_OPERATOR_PERMISSION_CEILING` still blocks `can_spawn=True` and `can_use_wallet=True` grants. |
| **`src/host/`** | |
| `server.py` | Mesh FastAPI app — 98 endpoints (`@app.*` decorators), all permission-checked. `_RATE_LIMITS` (18 entries). `/mesh/agents/{id}/edit-soft` accepts every editable field (soft + hard); receipt TTL is field-aware via `_ttl_for_field()`: `_CHANGE_TTL_SECONDS=300` for soft fields, `_HARD_CHANGE_TTL_SECONDS=1800` for hard fields. `pending_actions` table is now only for deletion confirmations (5-min TTL, mesh-side). Legacy `/mesh/agents/{id}/propose` + `/mesh/config/confirm` endpoints retained for in-flight pending rows and SDK back-compat. VNC reverse proxy with agent token rejection. Localhost validation for `x-mesh-internal`. `_require_operator_or_internal` permission tier between "any authenticated agent" and loopback-only. `system_metrics` (operator-or-internal) surfaces `blackboard_cross_project_total` (process-lifetime read/write counter, observability-only) and `tool_denials_24h` (24h rolling counter, categories `auth`/`scope`/`role`/`permission`/`rate`). Per-agent fields on metrics: `per_agent_cost_today_usd`, `per_agent_cost_vs_yesterday_ratio`, `outcome_rejected_24h_count`, `execution_failures_24h_count`, `stale_tasks_24h_count`. `GET /mesh/agents/{agent_id}/stale-tasks?threshold_hours=N` (operator-only) returns up to 5 oldest non-terminal tasks. |
...
9. **Monolithic server files.** `dashboard/server.py` (143 endpoints) and `host/server.py` (98 endpoints) are single function-scoped definitions.
...
14. **Tab IDs are frozen for URL stability.** The four top-nav tab IDs (`chat`, `fleet`, `workplace`, `system`) are load-bearing — they appear in URL paths, JS state vars, and dashboard endpoints. User-facing labels diverge: `fleet` renders as "Teams" (after the project→team rename in PR 2; was "Team" pre-rename), `workplace` renders as "Work", `system` renders as "Settings". Renaming the IDs would break deep-links and persisted preferences.
...
17. **Project → team rename in progress (PR 2 of 3).** The domain term flipped from "project" to "team": the workspace shared-context file is now `TEAM.md` (legacy `PROJECT.md` resolves as a fallback), the fleet-management dir is `config/teams/` (legacy `config/projects/` resolves via a startup-migrator-created symlink), and the operator surfaces canonical `*_team` tools (`create_team`, `inspect_teams`, `add_agents_to_team`, …) alongside the legacy `*_project` aliases. Every back-compat invariant in the PR-2 spec is preserved through PR 3: `MeshClient.project_name` is a property aliasing `team_name`, `AgentPermissions.can_manage_projects` mirrors `can_manage_teams` via a model-validator, `WebSocket` lifecycle emits fire under both `project_*` and `team_*` names, JSON responses include both `project_id` and `team_id` keys, `OPENLEGION_TEAM_SCOPE_MODE` falls back to `OPENLEGION_PROJECT_SCOPE_MODE`, `OPENLEGION_MAX_TEAMS` falls back to `OPENLEGION_MAX_PROJECTS`. PR 3 will sunset every alias on its own deprecation timeline. The frontend is intentionally only half-flipped in PR 2 (label change + tab ID `fleet` frozen per Constraint #14) — the Alpine state rename lands in PR 3. The orchestration DB `tasks.project_id` column rename is gated behind `OPENLEGION_TEAM_MIGRATION_RENAME_DB=1` (off by default this PR; tested but unwired so PR 3 can flip a single switch).
