...
14. **Tab IDs are frozen for URL stability.** The four top-nav tab IDs (`chat`, `fleet`, `workplace`, `system`) are load-bearing — they appear in URL paths, JS state vars, and dashboard endpoints. User-facing labels diverge: `fleet` renders as "Team", `workplace` renders as "Work", `system` renders as "Settings". Renaming the IDs would break deep-links and persisted preferences.
...
16. **Task auto-close is failure-only.** When the wake chain carries `x-task-id`, `loop.chat` / `loop.chat_stream` transitions `pending → working` at chat start (so the next status update is allowed by the state machine) and auto-closes ONLY for failure modes the agent can't declare itself: `failed(max_iterations_reached)` on tool-round exhaustion, `failed(<exception>)` on raised exceptions, `cancelled(agent_chat_cancelled)` on client/server cancellation. Clean chat completion does NOT auto-close as `done` — the chat handler runs whether the LLM did real work or just emitted a one-line ack, so treating chat-end as task-completion was lying. Agents must call `complete_task(task_key=...)` explicitly to declare done. The wake message in `_hand_off_v2` includes the task_id and prompts the recipient to call `complete_task` when finished. Back-edge events go to the originating agent's blackboard at `inbox/{agent}/task_event/{id}` with a 7-day TTL; surfaced via `check_inbox`. Humans still get the lane-worker auto-notify path (no back-edge); self-handoffs (`origin_user == assignee`) skip the back-edge to keep an originator's inbox clean. Tasks that an agent forgets to complete will dangle in `working` — a stale-task reaper is the right cleanup mechanism (separate concern).
