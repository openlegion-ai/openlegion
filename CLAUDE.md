...
9. **Project→team rename completed 2026-05.** Permanent zero-cost shims kept (type alias, `*_project` mesh-client proxies, CLI `--project` flag, env var fallbacks, `PROJECT.md` read fallback, `project_id` dict keys); operator `*_project` tools are recoverable error stubs.
