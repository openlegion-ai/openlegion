...
| `src/host/mesh.py` | Blackboard (SQLite WAL), PubSub, MessageRouter (cross-team block via injected `team_resolver`), `audit_log` with `undoable` + `archived`. |
