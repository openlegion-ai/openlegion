...
| `workspace.py` | Persistent markdown workspace. Bootstrap files: TEAM.md, SYSTEM.md, INSTRUCTIONS.md, SOUL.md, USER.md, MEMORY.md, INTERFACE.md. Also manages HEARTBEAT.md, daily logs, learnings. |
...
- `OPENLEGION_MAX_AGENTS`, `OPENLEGION_MAX_TEAMS` for plan limits
- `OPENLEGION_LOG_FORMAT=json` for production
- `OPENLEGION_BROWSER_MAX_CONCURRENT` — per-service cap on simultaneous Camoufox instances. **Startup-only**; restart the browser service to apply changes.
...
17. **Project → team rename completed (2026-05).** Domain term flipped end-to-end; minimal shims retained: `MeshClient.*_project` proxies, `AgentPermissions.can_manage_projects` validator, `tasks.project_id` dict-key emission alongside `team_id`. Blackboard storage prefix `projects/{name}/` and `target_kind="project"` on `pending_actions` are internal namespaces, not domain vocabulary. See `src/host/team_migration.py` for the idempotent startup migrator (scheduled for removal in the next release).
...
2026-05-19 — Work-tab persistent sub-nav + always-summaries default. The PR-B landing logic conditioned on team/summary count (summaries view only when ≥1 team or ≥1 summary existed; kanban otherwise). That made the UI scale-dependent — the user's pain wasn't "30 agents specifically," it was the per-task firehose at any scale. Dropped the conditional in `_applyDefaultHomeTab`: summaries is now the unconditional default landing regardless of fleet size. Empty state ("No summaries yet — ask operator") is the consistent fresh-install experience. Added a persistent `Summaries / Kanban / Activity` sub-nav tab bar at the top of the Work tab — `border-b-2` accent on the active tab, `role="tablist"` for a11y, single-click switch between all three views. The legacy "View activity feed →" / "View team summaries →" / "View kanban →" footer links + the "Back to Work" activity-back button were removed (the tab bar replaces both). The contextual filter-state "Back to team summaries" button + filter chip on the kanban view STAYED — it carries filter state (clearing `workplaceTeamFilter` + flipping tab), which the tab bar alone doesn't. Two test groups updated: `test_section_testids_present` now asserts the three `workplace-subnav-*` testids instead of the obsolete `board-view-activity`; `test_subtab_bar_replaces_legacy_workplace_tab` replaces `test_subtab_bar_removed` to also assert all three `switchHomeTab(...)` calls are present; `test_activity_subpage_reachable_via_subnav` replaces the legacy-footer-link assertion. 6311 fast suite tests pass.

2026-05-19 — Work-summaries cron bootstrap + defensive cleanup (closes the deferred item from #945). New `CronScheduler.ensure_summary_job(scope_kind, scope_id, schedule=None)` + `find_summary_job(scope_kind, scope_id)` — idempotent, matches on `tool_name="compose_work_summary"` plus the scope pair embedded in `tool_params` JSON. Default schedule is `CronScheduler.DEFAULT_SUMMARY_SCHEDULE = "0 9 * * *"` (daily 9am). Per-team cadence override lives in `team.settings.summary_schedule` (the `TeamMetadata.settings` extension dict, not a top-level field — keeps the schema pinned). New `RuntimeContext._reconcile_work_summary_jobs()` runs alongside `_reconcile_heartbeats()` in `_start_background()`: iterates active teams via `_load_projects()`, ensures one summary cron per team, AND prunes summary jobs whose scope_id no longer exists or whose team is archived. Each cron tick fires `compose_work_summary` directly as a `tool_name` invocation (no LLM round-trip per fire — deterministic + cheap). Defensive cleanups bundled in: `MAX_NARRATIVE_CHARS=32_000` cap enforced at the store layer and the mesh endpoint (HTTP 413), `generated_by` required-not-empty, `_summariesRefetchDebounce` (250ms) coalesces N back-to-back `work_summary_created` WS events from a multi-team cron tick into ONE list re-fetch, and `loadWorkplaceSummaries` clears any `_pendingExternalTimer` from old row objects before replacing the array so detached deferred-apply callbacks can't mutate orphan rows. 14 new cron tests + 3 new store boundary tests; 6309 fast suite tests pass.

2026-05-19 — Work tab summary-card UI redesign (PR-B). Replaces the per-task firehose default at 30+ agent scale with operator-composed team summary cards. New `homeTab='summaries'` sub-route (`/home/summaries`, deep-linkable). Auto-default lands on summaries when ≥1 team exists; falls back to kanban for solo / single-team fleets. Per-card UI: scope/period header + markdown-rendered narrative + metrics row + recommendations list + 👍/➖/👎 rating buttons + inline feedback box for 👎 (mirrors per-task outcome pattern). Markdown rendering is a tiny inline helper (`renderSummaryMarkdown` in `app.js`) that HTML-escapes input then handles `## H2`, `**bold**`, `- bullet` — the shapes `compose_work_summary` emits — without pulling in a third-party renderer. "Expand task list →" drills from a summary card into the kanban pre-filtered to that team (sets `workplaceTeamFilter` + flips `homeTab` to kanban). Kanban now shows a "Back to team summaries" link + filter chip when entered via that path. WebSocket handlers added for `work_summary_created` (re-fetches list to avoid racing the store-write) and `work_summary_rated` (live-syncs the rated row). `_applyDefaultHomeTab()` runs after `loadWorkplace()` settles so the auto-default sees the loaded team count; `homeTabUserChosen` flag prevents the auto-default from overriding an explicit user toggle or a deep link. 11 new dashboard API tests pin the proxy routes. 6290 fast suite tests pass.

2026-05-19 — Work summaries backend (PR-A, foundation for the summary-driven Work tab redesign). New `WorkSummariesStore` (`src/host/summaries.py`, SQLite WAL, 30-day retention with opportunistic reap) persists one row per `(scope_kind, scope_id, period_start)` triple — scope_kind ∈ `{team, solo}` covers project-attached and standalone agent work. Rating model mirrors per-task outcomes: `accepted` (👍) / `acknowledged` (➖) / `rework` (👎); first-rating timestamp preserved across edits via COALESCE, table locks the row after `RATING_EDIT_WINDOW_SECONDS=86400`. `compose_work_summary` operator skill (`src/agent/builtins/operator_tools.py`) composes deterministically from `team_summary` data + prior rated feedback — no LLM round-trip, reproducible, cron-cheap. Mesh endpoints: `POST/GET /mesh/work-summaries`, `GET/POST /mesh/work-summaries/{id}` (+ `/rating`); operator-or-internal gate on create + rate; scope-filtered visibility on read (`_can_read_summary` extends `_is_project_member` semantics to the solo scope = caller-self). Dashboard API proxy: `GET /api/workplace/summaries`, `GET /api/workplace/summaries/{id}`, `POST /api/workplace/summaries/{id}/rating`. WebSocket event literals added: `work_summary_created`, `work_summary_rated`. `compose_work_summary` added to `_OPERATOR_ALLOWED_TOOLS` (count → 49) and to the dashboard's `verbForTool` map. 57 new tests (store unit + endpoint integration + skill composition) + 6276 existing tests pass. Cron bootstrap deferred to a follow-up — operator manually invokes the skill via heartbeat / user chat until the daily cron lands.

2026-05-18 — Operator trust tier + structured denial logging. `_record_denial(category, *, caller, target, gate, extra)` now emits a structured `logger.warning("mesh denial: ...")` on every 14-site denial (5 auth, 1 role, 6 permission, 3 scope, 1 rate inside `_check_rate_limit`); the counter behavior is unchanged but the gate that fired is now grep-able from the mesh log alone. Caller-side fix: `src/agent/mesh_client._raise_with_body(response)` replaces every `response.raise_for_status()` (79 sites) — appends the body's `detail` field to the raised `httpx.HTTPStatusError` so coordination skills surface the gate reason in their failure envelopes (`wake_failed`, `create_failed`, etc.) instead of a bare "403 Forbidden". New `_caller_is_operator(caller, request)` predicate (`src/host/server.py`) is the single source of truth for the operator trust tier; consolidated 20 `caller != "operator" and not _is_internal_caller(request)` archive/edit guards + 13 inline `caller == "operator"` scope/inbox short-circuits into one helper call. Operator short-circuits applied at every coordination/management gate per the proposal's "can_* family more broadly" intent: `/mesh/wake` `can_message`, `/mesh/message` `can_message`, `_resolve_browser_target` delegation `can_message`, all 6 blackboard read/write/list/watch/claim/delete gates, `/mesh/publish` `can_publish`, `/mesh/subscribe` `can_subscribe`, `/mesh/api` + `/mesh/api/stream` `can_use_api`, `/mesh/cron` create/update/delete `can_manage_cron`, `/mesh/spawn` `can_spawn`, `/mesh/fleet/apply` + agent-create `can_manage_fleet`, `/mesh/tasks` create `can_route_tasks`, agent-history + agent-profile `can_message`. Carve-out left fully gated (operator's permission grants apply like any worker): `can_use_wallet*`, `can_access_credential` (cred-value reads), `can_manage_vault`, `can_browser_action` (drive actions). The static-inspection regression test `test_operator_still_gated_surfaces_not_in_bypass_grep` pins the carve-out boundary against accidental future widening. Help-action surface preserved — `request_browser_login` / `request_captcha_help` / `request_credential` already use per-target gates (`can_browser_action(agent_id, action)` where `agent_id` is the effective target, not the caller) and operator's own `browser_actions` grant covers them, so no carve-in needed. Cleanup: 4 internal `_PROJECT_SCOPE_MODE` references flipped to canonical `_TEAM_SCOPE_MODE` (alias + env-var fallback retained — `test_rollout_defaults.py` reads it directly + 40+ tests set `OPENLEGION_PROJECT_SCOPE_MODE`). Boot-time fail-closed gate: under `_TEAM_SCOPE_MODE=enforce` and empty `auth_tokens` the mesh raises `SystemExit` to refuse boot — without tokens the `X-Agent-ID` header is unverifiable and the operator trust short-circuit becomes forgeable. Production CLI populates `runtime.auth_tokens` during agent setup BEFORE `create_mesh_app`, so a non-empty dict is the normal boot state and an empty dict signals misconfiguration. Tests bypass via `OPENLEGION_SKIP_TRUST_TIER_BOOT_GATE=1` set globally in `tests/conftest.py` — explicit env-var signal, not an ambient `"pytest" in sys.modules` check (which would silently misfire if a production process transitively imported pytest via coverage or CI tooling). To run dev mode without auth, set `OPENLEGION_TEAM_SCOPE_MODE=warn`. `_OPERATOR_PERMISSION_CEILING` in `operator_tools.py` is unchanged — the bypass is at gate-evaluation time, not grant time, so operator still cannot grant `can_spawn=True` or `can_use_wallet=True` to a created agent. 19 new tests in `test_operator_trust_tier.py` pin every bypassed gate + every still-gated surface + the X-Agent-ID-forgery rejection path + `_record_denial` structured logging + the `_raise_with_body` JSON-detail / text-body / passthrough paths. 6201 existing tests still pass.

2026-05-18 — Dropped the legacy blackboard-dict coordination path. `hand_off` / `check_inbox` / `update_status` / `complete_task` (`src/agent/builtins/coordination_tool.py`) are now v2-only — the 4 dual-path functions collapsed to single bodies routing through `mesh_client.create_task` / `list_task_inbox` / `set_task_status`. Removed: `mesh_client.orchestration_v2_enabled()` + cache, `_ORCHESTRATION_TASKS_V2` env-flag guard on `tasks_store` construction (now unconditional), `_require_tasks_v2()` helper, `/mesh/orchestration/status` endpoint, `_orchestration_v2_on()` gate helpers in `operator_tools.py` (5 callsites) and `dashboard/server.py` (9 callsites), CLI's "Board disabled" fallback in `cli/main.py`, the `_TASKS_V2_DISABLED` error message, the `_STANDALONE_ERROR` constant on `coordination_tool.py` (unrelated `_STANDALONE_ERROR` on `mesh_tool.py` retained — different surface). `migrate_blackboard_to_tasks` still auto-runs at mesh startup so existing fleets convert any remaining legacy data once. The v2 rollout was the default since #835 (May 2); operator audit confirmed no production fleet was on `OPENLEGION_ORCHESTRATION_TASKS_V2=0`. Net: −300 LoC removed including duplicated failure-envelope code (Bug H now lives in one place per tool), −47 obsolete tests deleted, 1 helper renamed (`_v2_task_to_legacy_dict` → `_task_to_inbox_row`). 6167 tests pass, lint clean.
...
2026-05-18 — Bug F regression follow-up: dropped the `iterations_executed > 1` precondition from the lazy-completion guard after operator hit a trend-scout task that completed with text-only on iter 0 (the iter-0 nudge is gated on `available_tools` and silently skips when empty/None). Guard is now `tool_calls_count == 0 AND not structured_final` — fires on iter 0 too. **Codex r9 follow-up**: extended the same guard to `loop.chat()`'s `task_id` auto-close site so handoff tasks dispatched through the lane → `/chat` path (the actual production handoff route) can't auto-close as `done` when the LLM responds with text-only chatter and zero tool calls — that path used to bypass the `execute_task` guard entirely. Extracted `_is_structured_final` to a helper method so both guards share one source of truth for the documented `{"result": {...}}` contract. Legitimate one-iteration noop tasks must return that contract; chat-path nudge is unchanged (chat doesn't have an iter-0 nudge — first text-only is final). 7 new tests cover both paths.

2026-05-18 — Bug H follow-up to Bug G. Operator hit a live case where `seo-strategist` called `hand_off` targeting `page-writer`, `mesh_client.create_task` raised, and the agent still reported "Brief completed" in its final reply — page-writer never received the task. Root cause: the Bug G fix only hardened the `wake_agent` failure envelope. The other three hand_off failure paths (v2 `create_task`, v2 artifact `write_blackboard`, legacy task `write_blackboard`, legacy output `write_blackboard`) still returned bare `{"error": "Failed to create task: {e}"}`. Mirror the Bug G shape on all four sites: `handed_off=False`, distinct flag (`create_failed` / `output_write_failed`) + paired detail field (`create_error` / `write_error`), explicit `error` with "MUST NOT report success", directive `recovery_hint` with "DO NOT mark this work as complete" + "DO NOT retry without verifying via check_inbox" (post-commit failures may have left a partial row — wording deliberately softer than wake_failed's absolute "task IS queued" since here the row may or may not exist). Extended the same directive shape to `update_status` / `complete_task` via a new `_failed_transition_envelope` helper (Codex r5 — same silent-failure mode on terminal transitions). Helper places sentinel keys (`error`, `recovery_hint`, `detail`, dynamic `<kind>` flag) AFTER `extras` in the merge order so a future caller cannot shadow them — pinned by `test_extras_cannot_shadow_sentinel_keys`. Reserved-key surface documented in `_RESERVED_ENVELOPE_KEYS`. Credential redaction (`redact_text_with_urls`) applied to all four new envelope paths + the two new transition envelopes, matching the existing wake_error precaution. Loop-side defense-in-depth deliberately deferred per operator's original Option-B-only recommendation; the hardened envelope has held in practice since Bug G.

2026-05-18 — Closed two autonomous-pipeline failure modes operator hit running the E2E SEO flow (trend-scout → seo-strategist → page-writer → page-validator → dev-publisher). **Bug F (lazy completion)**: `execute_task` terminal text-only branch now downgrades to `status='failed'` with `error="no_action_taken: ..."` when `tool_calls_count == 0 AND not structured_final` (the `iterations_executed > 1` precondition was dropped in the regression follow-up; see entry above). `tool_calls_count` is a local function-scope counter (seeded from `messages`, incremented at dispatch, conservatively bumped to 1 on resume-from-checkpoint with zero — the post-compaction case where pre-checkpoint tool history is gone). `structured_final` requires `json.loads(content)` to be a dict with a `"result"` key whose value is a NON-EMPTY dict — rejects `{"result": "chatter"}` and `{"result": {}}` bypass attempts. **Bug G (silent peer hand_off)**: both v2 (`_hand_off_v2`) and legacy (`hand_off`) wake-failure envelopes now carry an explicit `error: "wake_failed: peer '{to}' did not wake (...). The task is queued (task_id={X} | task_key={Y}) but the recipient has not been notified — you MUST NOT report success..."` key + a directive `recovery_hint` that embeds the task locator and explicitly says `DO NOT retry hand_off` (each call creates a new SQLite row → orphan duplicates) and `DO NOT mark this work as complete`. All pre-existing keys preserved (additive change). Operator playbook updated with the new envelope shape so the operator LLM stops papering over wake failures with "task complete" summaries. Skipped: watchdog for stale pending tasks (operator's stronger alternative — separate PR if envelope hardening isn't sufficient).
