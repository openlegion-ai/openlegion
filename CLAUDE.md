...
| **`src/agent/`** | |
| `loop.py` | Agent execution loop (task + chat mode). `MAX_ITERATIONS=20`, `CHAT_MAX_TOOL_ROUNDS=30`, `CHAT_MAX_TOTAL_ROUNDS=200`, `_MAX_SESSION_CONTINUES=5`, `HEARTBEAT_MAX_ITERATIONS=12`. Env-var bounds clamped via `_clamp_env()`. |
...
9. **Monolithic server files.** `dashboard/server.py` (~6255 lines, 143 endpoints) and `host/server.py` (~7103 lines, 98 endpoints) are single function-scoped definitions.
