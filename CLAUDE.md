...
| `mesh_client.py` | Agent-side HTTP client for mesh communication. `wake_agent` and `create_task` both accept an optional `origin: MessageOrigin` and merge `origin_header(origin)` into request headers when provided. |
...
| `fleet_tool.py` | Operator-only fleet management tools (`list_templates`, `apply_template`). `apply_template` accepts optional `agent_overrides: dict[str, dict]` (v1 fields: `{model, instructions}`) validated upfront by the mesh route before any agent is created. |
...
- **Denial counter (observability).** `tool_denials_24h` on `/mesh/system/metrics` counts denials in five frozen categories — `auth` (missing/invalid bearer), `scope` (out-of-project / out-of-roster), `role` (operator-only denied to worker), `permission` (`permissions.can_*()` returned False), `rate` (rate limiter rejected). 24h rolling window, lazy day rollover, missing keys read as zero. Operator-or-internal gated. No enforcement effect — pure observability for the operator heartbeat.
...
9. **Monolithic server files.** `dashboard/server.py` (~6255 lines, 135 endpoints) and `host/server.py` (~7089 lines, 96 endpoints) are single function-scoped definitions.
