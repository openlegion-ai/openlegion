...
| `await_task_event` | `task_id`, `timeout_s` (default 45, cap 90), `poll_interval_s` (default 3) | Briefly wait (≤90s) for one task to reach a terminal status (`done` / `failed` / `blocked` / `cancelled`), polling its **durable task status**, else return `timed_out`. Demoted to a short in-turn pickup check (delegate-and-subscribe) — the chain watcher delivers end-to-end outcomes; the operator hands off and releases. |
| `inspect_teams` | `detail` (default `"names"`; values: `names` / `status` / `full`), `team_name` (default "") | Look up teams. With no `team_name`, `names` returns name+description, `status` returns task-count rollups. Passing a `team_name` returns the full record — including the team's `north_star` / `success_criteria` goal and its `budget_daily_usd` / `budget_monthly_usd` envelope, so you can read back a goal or budget you set. |
| `inspect_team_spend` | `team_name`, `period` (default `"today"`; values: `today` / `yesterday` / `week` / `month`) | Aggregate team spend for the period: total cost + tokens, the daily/monthly budget envelope (null = unlimited), and a per-member cost/token breakdown — answers "how much has this team spent" / "who is burning the most budget" without fanning out `inspect_agents` per member. |
| `manage_team` | `action` (enum: `archive` / `unarchive` / `propose_delete` / `set_budget`), `team_name`, plus `daily_usd` / `monthly_usd` for `set_budget` | Lifecycle + budget for a team. `propose_delete` requires the team to already be archived (returns a confirmation nonce). `set_budget` sets the team's aggregate spend envelope (enforced across all members' LLM calls) — an **omitted** period keeps its current value; pass a number to set it, or `0`/`null` to make that period unlimited (so `daily_usd` alone tightens the daily cap and leaves monthly untouched). |
...
| `update_team_context` | `team_name`, `context` | Write the team's `## Context` section of `TEAM.md` — **section-scoped** (leaves `## Goal`, `## User Preferences`, and brief sections intact) and pushed to running members. |
| `update_team_brief` | `team_name`, `section` (heading without `##`, e.g. `User Preferences`), `content` (≤2000 chars) | Write ONE named `## {section}` block of `TEAM.md` — the way to propagate durable fleet-wide knowledge (tone, audience, formats, constraints) to every member at once. Section-scoped; the updated file is pushed to running members. |
| `set_team_lead` | `team_name`, `agent_name` | Appoint an existing team member as the team's lead — the accountability owner who receives standup duty, goal-coverage/blocked-task escalation, and advisory review verdicts. The agent must already be a member; the operator cannot be lead and a solo team needs none. Team data, not a permission tier — grants zero permission elevation. (Non-solo teams also get a lead auto-appointed on create/add and auto-re-appointed on any departure.) |
| `set_team_goal` | `team_name`, `north_star`, `success_criteria` (default []) | Save the team's north star (≤2000 chars) and up to 10 success criteria (each ≤200 chars). Persists to the team store, mirrors into the `## Goal` section of `TEAM.md`, and live-pushes to running members — the redirect takes effect on each member's **next turn** (an in-flight task finishes under its current context). Pass empty values to clear. No confirmation required — call proactively whenever the user describes a team goal. |
| `set_agent_goals` | `agent_id`, `goals` (full replacement, ≤5 entries, each ≤300 chars; `[]` clears) | Assign standing goals to ONE worker (not the operator). Goals ride that agent's every prompt under `## Your Current Goals` and make idle heartbeats pursue them instead of sleeping. For worker direction — fleet/business goals live in `manage_goals`. |
...
| `assess_team_progress` | `team_name`, `since` (default `"14d"`; ISO timestamp or duration like `"7d"` / `"30d"`) | Compose a goal-vs-evidence bundle to judge delivery: `north_star` + `success_criteria` alongside the team's actual completed work (via `get_team_outputs`), task-count rollups, and current blockers — all worker text sanitized. Returns `assessment_guidance` (a per-criterion `met` / `on_track` / `at_risk` / `no_evidence` / `needs_external_metric` rubric); verify any measurable criterion against the real external metric, never a guessed figure. Replaces the raw `summarize_team_progress` rollup on the operator surface. |
