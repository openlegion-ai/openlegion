...
| `can_spawn` | boolean | Whether this agent can spawn ephemeral fleet agents via `spawn_fleet_agent`. Default: `true` for new named agents; ephemeral spawned agents resolve to `false` so spawns can't recurse. |
...
| `BROWSER_AUTH_TOKEN` | -- | Bearer token the mesh uses to call the browser service (`/browser/*` endpoints). When `MESH_AUTH_TOKEN` is set and `BROWSER_AUTH_TOKEN` is missing, the browser service raises `RuntimeError` at startup unless `BROWSER_AUTH_INSECURE=1` is set. **Fleet-wide superuser credential:** the browser service has no per-agent identity — it trusts the `agent_id` in the URL path, so any holder of this token can drive ANY agent's browser. Keep it in the mesh process only; never inject it into an agent container's env and never log it. See `docs/security.md` → "`BROWSER_AUTH_TOKEN` is a fleet-wide superuser credential" (M5). |
...
| `OPENLEGION_COORDINATION_DAILY_CAP_USD` | `2.0` | Per-agent daily spend cap (USD) for **coordination** LLM traffic — calls whose requested model matches `llm.utility_model` (B2 spend split, `src/shared/limits.py`). Coordination calls skip the per-agent work budget and the team envelope and are gated by this cap alone; a blocked call fails with a distinct "Coordination budget exceeded" error. Clamped to `[0.0, 100.0]`; **`0` is valid and blocks the coordination tier entirely** (operator kill-switch). Invalid values fall back to the default. No effect when `llm.utility_model` is unset (no call ever classifies as coordination). |
