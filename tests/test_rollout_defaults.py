...
def test_team_scope_mode_default_is_enforce(monkeypatch):
    """With the env var unset, ``_TEAM_SCOPE_MODE`` is ``enforce``."""
    monkeypatch.delenv("OPENLEGION_TEAM_SCOPE_MODE", raising=False)
    server_module = _reload_server()
    try:
        assert server_module._TEAM_SCOPE_MODE == "enforce"
    finally:
...
def test_team_scope_mode_can_be_warn(monkeypatch):
    """``OPENLEGION_TEAM_SCOPE_MODE=warn`` is the rollback kill-switch."""
    monkeypatch.setenv("OPENLEGION_TEAM_SCOPE_MODE", "warn")
    server_module = _reload_server()
    try:
        assert server_module._TEAM_SCOPE_MODE == "warn"
    finally:
        monkeypatch.delenv("OPENLEGION_TEAM_SCOPE_MODE", raising=False)
        _reload_server()


def test_invalid_team_scope_mode_falls_back_to_enforce(monkeypatch, caplog):
    """An unknown value coerces to the new default (``enforce``)."""
    import logging

    monkeypatch.setenv("OPENLEGION_TEAM_SCOPE_MODE", "bogus")
